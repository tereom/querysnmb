...
#' @importFrom magrittr %>%
#' @export
queryFiles <- function(database, file_type, state = "all", organization = "all", cgl_id = "all", 
    year_visit = 2010:2016, month_visit = 1:12, photo = TRUE, video = TRUE, ultrasonic = FALSE, 
    original = FALSE) {
    
    # el método actual puede ocasionar problemas en el caso original = TRUE si hay dos
    # archivos con el mismo nombre los resultados fallarán.  Se puede mejorar el código
    # haciendo lazy evaluation y ejecutamos los queries hasta que filtramos.  connect
    # to database (snmb)
    
    cgl_table_aux <- dplyr::tbl(database, "conglomerado_muestra") %>% dplyr::collect()
    ...
    cgl_table <- cgl_table_aux %>% dplyr::mutate(anio = lubridate::year(fecha_visita), 
        mes = lubridate::month(fecha_visita)) %>% dplyr::filter(estado %in% state, 
        institucion %in% organization, nombre %in% cgl_id, anio %in% year_visit, mes %in% 
            month_visit) %>% dplyr::select(conglomerado_muestra_id = id, estado, institucion, 
    ...
    if (nrow(cgl_table) == 0) {
        ...
        if (original) {
            ...
            cgl_paths_table <- searchPaths(database, cgl_table, file_type, photo, video, 
                ultrasonic)
        }
        if (nrow(cgl_paths_table) == 0) {
            print("No hay achivos que cumplan los requisitos solicitados, revisar conexión al servidor (cluster/madmex).")
            final_table <- NA
        } else {
            final_table <- dplyr::collect(dplyr::tbl(database, "sitio_muestra")) %>% 
                dplyr::filter(sitio_numero == "Centro") %>% dplyr::right_join(cgl_paths_table, 
                by = "conglomerado_muestra_id") %>% dplyr::mutate(file_type = file_type, 
                lat = lat_grado + lat_min/60 + lat_seg/3600, lon = ifelse(lon_grado > 
                  0, lon_grado + lon_min/60 + lon_seg/3600, -(lon_grado - lon_min/60 - 
                  lon_seg/3600)))
            if (original) {
                final_table <- final_table %>% dplyr::select(file_type, estado, institucion, 
                  cgl_id, fecha_visita, lat, lon, path, archivo_nombre_original)
            } else {
                final_table <- final_table %>% dplyr::select(file_type, estado, institucion, 
                  cgl_id, fecha_visita, lat, lon, path)
            }
        }
    }
    final_table
...
searchPaths <- function(database, cgl_table, file_type, photo = TRUE, video = TRUE, 
    ultrasonic = FALSE) {
    # returns a data.frame of path per cgl_id, year_visit, month we create paths
    # according to the directory structure of media
    ...
    if (file_type == "recordings") {
        folder <- ifelse(ultrasonic, "grabaciones_ultrasonicas", "grabaciones_audibles")
        pattern <- "."
    } else {
        folder <- "fotos_videos"
    ...
    # find directories
    dirs_search <- paste(ruta_archivos_cluster, cgl_table$expr, "/", folder, sep = "")
    ...
    paths_df <- data.frame(expr = stringi::stri_extract_first(str = files_search, regex = "[:digit:]+/[:digit:]+_[:digit:]+"), 
        path = files_search, stringsAsFactors = FALSE)
...
searchPathsOriginal <- function(database, cgl_table, file_type, photo = TRUE, video = TRUE, 
    ultrasonic = FALSE) {
    # returns a data.frame of path per cgl_id, year_visit, month we create paths
    # according to the directory structure of media create a list of potential
    # directory paths using cross product
    ...
    paths_df <- data.frame(expr = stringi::stri_extract_first(str = files_search, regex = "[:digit:]+/[:digit:]+_[:digit:]+"), 
        path = files_search, stringsAsFactors = FALSE)
    cgl_paths <- dplyr::left_join(paths_df, cgl_table, by = "expr") %>% dplyr::mutate(archivo_nombre_aux = basename(path))
    if (file_type != "recordings") {
        cgl_paths <- cgl_paths %>% dplyr::mutate(archivo_nombre_aux = substr(archivo_nombre_aux, 
            start = 4, stop = nchar(archivo_nombre_aux)))
    }
    tab <- dplyr::tbl(database, tab) %>% dplyr::collect() %>% dplyr::mutate(archivo_nombre_aux = gsub("(.*\\.).*\\.(.*\\.).*\\.", 
        "\\1\\2", archivo)) %>% dplyr::select(archivo_nombre_aux, archivo_nombre_original)
    cgl_name <- cgl_paths %>% dplyr::left_join(tab, by = "archivo_nombre_aux") %>% 
