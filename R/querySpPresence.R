...
querySpPresence <- function(database, noms, malla = NULL) {
  ...
  tr_ei <- dplyr::tbl(database,
    "transecto_especies_invasoras_muestra") %>%
    dplyr::collect() %>%
    dplyr::select(transecto_especies_invasoras_id = id,
  ...
  tr_he <- dplyr::tbl(database, "transecto_huellas_excretas_muestra") %>%
    dplyr::collect() %>%
    dplyr::select(transecto_huellas_excretas_id = id,
      conglomerado_muestra_id) %>%
    dplyr::left_join(conglomerado, by = "conglomerado_muestra_id")
  ...
  camara <- dplyr::tbl(database, "camara") %>%
    dplyr::collect() %>%
    dplyr::select(camara_id = id, sitio_muestra_id) %>%
  ...
  desagregado <- conglomerado %>%
    ...
    dplyr::mutate_each(dplyr::funs(naZero), contains("esp")) %>%
    dplyr::mutate(
      pres = (ei_esp + he_esp + ei_ex_esp + he_ex_esp + er_ex_esp +
        camara_esp) > 0,
      cgl_id = as.numeric(nombre)
      ) %>%
    dplyr::filter(pres) %>%
